...
3. Иначе рассматриваются два отрезка: $[a,c]$ и $[b,c]$. Из них выбирается такой, что функция в граничных точках имеет разный знак, _корень ищется внутри нового отрезка_.
